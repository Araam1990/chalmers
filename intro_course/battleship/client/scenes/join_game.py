...
class JoinGameMenu():
    ...
    def init_bg(self):
        ...
        text = font.render("Join Game", 1, (0,0,0))
    ...
    def init_buttons(self):
        ...
        self.buttons = [
            Button(button, ((self.size[0] - button_size[0]) // 2, 390), "Join", self.join),
    ...
    def init_events(self):
        events.sub("no room", self.no_room)
        events.sub("remove join error", self.remove_error)
        events.sub("room full", self.room_full)
        events.sub("username already used", self.username_being_used)
    ...
    def join(self):
        ...
        if not self.room_name:
            ...
            return
        client.send("join game", [client.id, self.room_name, client.username])
        client.receive()
    ...
    def set_error(self, text):
        self.error = Error(text, (self.size[0] // 2, 180), "remove join error")

    def remove_error(self):
        self.error = None

    def no_room(self):
        self.set_error("No room with that name")

    def room_full(self):
        self.set_error("The room is already full")

    def username_being_used(self):
        self.set_error("Username is already used")
